...
const AdminWishlistPage = () => {
    ...
    useEffect(() => {
        const fetchWishlist = async () => {
            try {
                const res = await axios.get(`${SERVER_URL}/api/admin-wishlist`, {
    ...
    const handleDelete = async (itemId) => {
        setRemoving(itemId);
        try {
            await axios.delete(`${SERVER_URL}/api/admin-wishlist/${itemId}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
            });
    ...
    const handleAddWishlistItem = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post(`${SERVER_URL}/api/admin-wishlist`, newWishlistItem, {
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
            });
